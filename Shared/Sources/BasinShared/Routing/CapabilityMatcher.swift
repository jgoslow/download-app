...
public enum CapabilityMatcher {
    ...
    private static let triggers: [(capability: String, keywords: [String])] = [
        ("create_task",     ["task", "ticket", "todo", "to-do", "to do", "bug", "issue", "follow up", "follow-up", "backlog", "jira", "card"]),
        ...
        ("capture_note",    ["note", "remember", "journal", "jot", "capture", "for the record"]),
    ]
    ...
    public static func match(_ transcript: String) -> [String] {
        ...
        let hits = triggers
            .filter { _, keywords in keywords.contains { text.contains($0) } }
            .map(\.capability)
