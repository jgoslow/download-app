...
public struct BasnSettings: Codable, Equatable, Sendable {
	public static let baseSoundEffectsVolume: Double = BasnCoreConstants.baseSoundEffectsVolume
	...
	public init(
		...
		soundEffectsVolume: Double = BasnSettings.baseSoundEffectsVolume,
		...
		wordRemovals: [WordRemoval] = BasnSettings.defaultWordRemovals,
	...
	public init(from decoder: Decoder) throws {
		self.init()
		let container = try decoder.container(keyedBy: BasnSettingKey.self)
		for field in BasnSettingsSchema.fields {
	...
	public func encode(to encoder: Encoder) throws {
		var container = encoder.container(keyedBy: BasnSettingKey.self)
		for field in BasnSettingsSchema.fields {
...
private enum BasnSettingKey: String, CodingKey, CaseIterable {
...
private struct SettingsField<Value: Codable & Sendable> {
	let key: BasnSettingKey
	let keyPath: WritableKeyPath<BasnSettings, Value>
	let defaultValue: Value
	let decodeStrategy: (KeyedDecodingContainer<BasnSettingKey>, BasnSettingKey, Value) throws -> Value
	let encodeStrategy: (inout KeyedEncodingContainer<BasnSettingKey>, BasnSettingKey, Value) throws -> Void

	init(
		_ key: BasnSettingKey,
		keyPath: WritableKeyPath<BasnSettings, Value>,
		default defaultValue: Value,
		decode: ((KeyedDecodingContainer<BasnSettingKey>, BasnSettingKey, Value) throws -> Value)? = nil,
		encode: ((inout KeyedEncodingContainer<BasnSettingKey>, BasnSettingKey, Value) throws -> Void)? = nil
	) {
		self.key = key
...
private struct AnySettingsField {
	let key: BasnSettingKey
	let decode: (KeyedDecodingContainer<BasnSettingKey>, inout BasnSettings) throws -> Void
	let encode: (BasnSettings, inout KeyedEncodingContainer<BasnSettingKey>) throws -> Void

	func decode(into settings: inout BasnSettings, from container: KeyedDecodingContainer<BasnSettingKey>) throws {
		try decode(container, &settings)
	}

	func encode(_ settings: BasnSettings, into container: inout KeyedEncodingContainer<BasnSettingKey>) throws {
		try encode(settings, &container)
	}
}

private enum BasnSettingsSchema {
	static let defaults = BasnSettings()
