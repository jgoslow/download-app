...
final class BasnSettingsMigrationTests: XCTestCase {
	func testV1FixtureMigratesToCurrentDefaults() throws {
		let data = try loadFixture(named: "v1")
		let decoded = try JSONDecoder().decode(BasnSettings.self, from: data)
		...
		XCTAssertEqual(decoded.soundEffectsVolume, BasnSettings.baseSoundEffectsVolume)
	...
	func testInitNormalizesDoubleTapOnlyWhenLockDisabled() {
		let settings = BasnSettings(useDoubleTapOnly: true, doubleTapLockEnabled: false)
	...
	private func loadFixture(named name: String) throws -> Data {
		guard let url = Bundle.module.url(
			forResource: name,
			withExtension: "json",
			subdirectory: "Fixtures/BasnSettings"
		) else {
