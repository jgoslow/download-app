...
actor ParakeetClient {
  ...
  private func candidateRoots() -> [URL] {
    ...
    let appCache = try? URL.basnApplicationSupport.appendingPathComponent("cache", isDirectory: true)
...
actor ParakeetClient {
  ...
  func ensureLoaded(modelName: String, progress: @escaping (Progress) -> Void) async throws {
    throw NSError(
      domain: "Parakeet",
      code: -2,
      userInfo: [NSLocalizedDescriptionKey: "Parakeet support not linked. Add Swift Package: https://github.com/FluidInference/FluidAudio.git and link FluidAudio to Basn."]
    )
  }
