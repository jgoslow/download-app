...
| 2 | no | 6s | "Speaking out loud is the best way to use Basn — but you can always switch to text at any time by hitting the keyboard icon at the bottom." | — |
...
## Phase G: Revise Setup Flow Prompts

The v1 setup flow prompts were scaffolding — they established the data model and flow mechanics but the content itself needs a full rethink before this ships to users.

### Problems with v1 prompts
- Timed prompts (2s, 6s) auto-advance before the user has read them
- "What does a typical day look like?" has no context — the user doesn't know what's relevant yet
- "What outcomes matter most to you?" is too abstract without grounding in what Basn actually does
- The prompt sequence feels like a config wizard, not a natural first experience
- 9 prompts is too many for a first-run flow

### Goals for v2
- Each prompt should feel like a question a thoughtful coach would ask, not a form field
- Prompts should build on each other — each answer gives context for the next
- The setup flow should double as the user's first genuine flow (captures something real)
- Target: 5–7 prompts, all optional with a natural speaking pace
- No timed auto-advance — let the user breathe

### Open decisions
- Should tool preference collection stay in the setup flow, or move to post-flow tool connect screens?
- Should early prompts be more "tell me about yourself" and let Castellum extract tool/workflow signals?
- Can the setup flow serve as an evergreen "morning kickoff" template, not just a one-time thing?

**This phase is primarily content/product work. User to draft the revised prompt sequence; engineering then replaces `FlowDefinition.setupPrompts` in `Hex/Models/BasinModels.swift`.**

---

## Decisions deferred to future plans

- **Real-time local intent routing** — The live `SFSpeechRecognizer` transcript stream is available during the flow *before* the session ends. This opens a meaningful optimization path:
  - Detect keywords and tool signals locally (zero Anthropic API calls) — e.g. "remind me", "add to Jira", tool names, meeting references — and pre-stage routing actions before the user even finishes speaking.
  - For simple captures where intent is already unambiguous from the transcript, the final Castellum call can be skipped or replaced with a lightweight rule-match, reducing cost and latency.
  - For complex captures, the pre-categorized, filler-stripped transcript can be sent to Claude instead of raw audio output — shorter context, lower token cost.
  - v1 uses the transcript only for display + "next" navigation. v2 should wire `FlowTranscriptionEngine.onSentenceComplete` into a lightweight local router that flags candidate actions in real time.

- **Real-time Castellum polling during flow** (v2): checking the transcript every 2-3s for prompt completion signals. v1 uses swipe/tap for manual advancement.
